...
export async function getProjectsFromNotion() {
  ...
  return response.results.map((page) => {
    ...
    return {
      ...
      images,
      githubUrl: props.GithubUrl?.url || null,
      liveUrl: props.LiveUrl?.url || null,
      version: getPlainText(props.Version?.rich_text) || null,
    };
  });
}
...
export async function getApprovedGuestbookEntries() {
  const dataSourceId = await getDataSourceId(process.env.NOTION_GUESTBOOK_DB_ID);
  ...
  return response.results.map((page) => ({
    ...
    message: getPlainText(page.properties.Message?.rich_text),
  }));
}
...
export async function addGuestbookEntry({ name, message }) {
  ...
  await notion.pages.create({
    ...
    properties: {
      Name: { rich_text: [{ text: { content: name.slice(0, 60) } }] },
      Message: { rich_text: [{ text: { content: message.slice(0, 300) } }] },
